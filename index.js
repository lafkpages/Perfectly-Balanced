...
// load google client
const googleClientID = '537840286289-i5pbg7omij5tlo5tsdduraolb41070fj.apps.googleusercontent.com';
...
function checkAchievements(vars, filterAwarded=false)
{
  ...
  for (let id of Object.entries(achievements.achievements))
  {
    ...
    if (filterAwarded && vars.achievements.includes(id))
      continue;
...
{
  if (!uid.startsWith(db_prefix_user))
    uid = db_prefix_user + uid;
...
async function sendWebhook(message, user = null) {
  ...
  try {
    const resp = await axios.post(`${process.env.DISCORD_WEBHOOK}?wait=true`, message, {
      headers: {
...
app.get('/', (req, res) => {
  ...
  console.log('IPADDR:', req.headers['x-forwarded-for'] || req.socket.remoteAddress);

  if (
    req.headers.host && (
      /(perfectly-balanced[a-z\-]*--luisafk|perfectly-balanced-[a-z\-]+\.luisafk)\.repl\.co/i.test(req.headers.host) ||
  ...
  getUsers().then(ids => {
    ...
    fs.readFile(__dirname + '/static/index.html', 'ascii', (err, data) => {
      ...
      fs.readFile(__dirname + '/static/important.css', 'ascii', (err, data2) => {
        if (err) {
  ...
  if (typeof req.body == 'string')
  {
    ...
    {console.log(req.body)
      getUser(id).then(user => {
        ...
        user.nick = req.body.trim().replace(/[\u200B\u200C\u200E\u200F\u2061\u2062\u2063\u2064]/g, '');
  
        const shortNick2 = user.nick.substr(0, 10);
  
        // increment name changes
        if (typeof user.nickChanges != 'number')
          user.nickChanges = 0;
...
app.post('/submit-score', (req, res) => {
  ...
  if (id) {
    getUser(id).then(user => {
      ...
      {
        console.log(`User "${shortNick}" beat level ${levelIdP} but has only unlocked level ${userLevelP}`);
        res.status(400);
      ...
      if (score > user.bestScore) {
        ...
        console.log(`Changed user "${shortNick}" best score from ${oldBestScore} to ${score}`);
        sendWebhook({
          embeds: [{
      ...
      if (!user.worstScore || score < user.worstScore) {
        ...
        console.log(`Changed user "${shortNick}" worst score from ${oldWorstScore} to ${score}`);
        sendWebhook({
          embeds: [{
      ...
      if ((!isLevel) || (isLevel && levelId == user.level && levelBeaten)) {
        ...
        coinsToReward = isLevel? (levelId == user.level? shop.levels.reward : 0) : Math.floor(score / 10) * coinsMultiplierByDifficulty[difficulty];
        ...
        if (coinsToReward > 0) {
          console.log(`Changed user "${shortNick}" coins from ${oldCoins} to ${user.coins}`);
        }

  
        if (isLevel && levelId == user.level && levelBeaten) {
          user.level++;
          console.log(`User "${shortNick}" beat level ${levelIdP} and unlocked level ${user.level + 1}`);
          sendWebhook({
            embeds: [{
              title: 'Level beaten!',
              fields: [{
      ...
      if (achs.length > 0)
      {
        console.log(`Awarded user "${shortNick}" achievements ${achs.map(ach => `"${achievements.achievements[ach].name}"`).join(', ')}`);
        sendWebhook({
          embeds: [{
            title: 'Achievements awarded!',
            fields: [{
              name: 'Achievements',
              value: achs.map(ach => `\u200b    \u2022 ${achievements.achievements[ach].name}`).join('\n'),
              inline: false
            }]
          }]
        }, user);
        ...
        for (const ach of achs) {
          ...
          switch (achievements.achievements[ach].rewardType) {
            ...
            default:
              ...
              console.log(`Awarded user "${shortNick}" ${achievements.achievements[ach].reward} coins for achievement "${achievements.achievements[ach].name}"`);
              break;
          }
        }
      }
      ...
      res.send({
        message: ((updatedBestScore? 'Updated best score' : '') +
          '\n' + (coinsToReward > 0? `Rewarded ${coinsToReward} coins` : '')).trim(),
        achievements: achs
      });
    });
  }
    ...
    if (req.body)
    {
      ...
      if (shelfId && itemId)
      {
        if (shelfId in shop.shelves)
        {
          ...
          {
            const buyingDisabled = shop.shelves[shelfId].buyingDisabled || shop.shelves[shelfId].items[itemId].buyingDisabled;
            ...
            else
            {
              getUser(id).then(user => {
                ...
                if (user.items?.includes(fullId)) {
                  ...
                  const price = shop.shelves[shelfId].items[itemId].price || shop.shelves[shelfId].price;
                  if (user.coins >= price) {
                    user.coins -= price;
...
app.post('/shop/select', (req, res) => {
  ...
  if (id)
  {
    ...
    {
      getUser(id).then(user => {
        if (shop.shelves[shelfId].selectingDisabled || shop.shelves[shelfId].items[itemId].selectingDisabled)
        {
          res.status(403);
...
app.get('/prize-wheel/daily/status', (req, res) => {
  ...
  if (id) {
    getUser(id).then(user => {
      if (!user.lastDailyPrize)
        user.lastDailyPrize = 0;
...
app.post('/googleToken', (req, res) => {
  ...
  googleClient.verifyIdToken({
    ...
    .then(ticket => {
      ...
      console.log(`User ${payload.email}${payload.email_verified? '✅' : ''} "${payload.name}" signed in with Google`);
      getUser(id).then(user => {
        sendWebhook({
          embeds: [{
...
function disconnect(socket, reason=null) {
  ...
  if (socket.id in connected_users)
    delete connected_users[socket.id];
}

io.on('connection', socket => {
  const cookieHeader = socket.handshake.headers.cookie || socket.handshake.headers['x-cookie'];
  ...
  const deathHandler = () => {
    ...
    let allDead = true;
      ...
      console.log('Received death but either user is not in a game or game doesn\'t exist');
      if (game_data[game_id]?.users instanceof Array) {
        io.sockets.sockets.forEach(socket => {
          if (game_data[game_id].users.includes(socket.id)) {
            disconnect(socket, 'There was a server error, please try again later');
            return;
          }
        });
      }
  ...
  socket.on('disconnect', () => {
    ...
    if (Object.entries(game_data[game_id].users).every(user => user[1].disconnected))
    {
...
server.listen(3000, () => {
  ...
  console.log(`https://replit-database-viewer.luisafk.repl.co/?url=${Buffer.from(process.env['REPLIT_DB_URL']).toString('base64')}\n`);
});
...
setInterval(() => {
  ...
  getUsers().then(ids => {
    ...
    Promise.all(users).then(users => {
      // sort by best scores
      users = users.sort((a, b) => (b[1].bestScore || 0) - (a[1].bestScore || 0));
      ...
      fs.writeFile(__dirname + '/leaderboard.json', json, 'ascii', err => {
        if (err)
          ...
          console.log('Saved leaderboard to file');
      });
    });
  });
}, 90000);